...
export const Filter = ({ currentView, currentFilter }) => {
    ...
    useEffect(() => {
        const storedView = localStorage.getItem('tasks_view');
        const storedFilter = localStorage.getItem('tasks_filter');
    ...
    const handleViewChange = (newView) => {
        setView(newView);
        localStorage.setItem('tasks_view', newView);
        const params = new URLSearchParams(searchParams.toString());
    ...
    const handleFilterChange = (newFilter) => {
        setFilter(newFilter);
        localStorage.setItem('tasks_filter', newFilter);
        const params = new URLSearchParams(searchParams.toString());
            ...
            <div className="flex items-center gap-4">
                ...
                <details className="dropdown">
                    ...
                    <ul className="menu dropdown-content bg-base-100 rounded-box z-1 w-52 shadow-sm">
                        ...
                        <li><a onClick={() => handleFilterChange("in_progress")}>In Progress</a></li>
                        <li><a onClick={() => handleFilterChange("completed")}>Completed</a></li>
                        <li><a onClick={() => handleFilterChange("on_hold")}>On Hold</a></li>
                        <li><a onClick={() => handleFilterChange("cancelled")}>Cancelled</a></li>
                        <li><a onClick={() => handleFilterChange("blocked")}>Blocked</a></li>
                    </ul>
                </details>
            </div>
